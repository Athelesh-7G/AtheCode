...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `athecode login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `athecode login`, set XAI_API_KEY, or add api_key to ~/.athecode/config.toml.";
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `athecode login` to authenticate.";
